...
export default function TaskCard({ taskd, onToggle }: TaskCardProps) {
  ...
  const isOverdue = taskDate < today && !taskd.status;
    ...
    >
      <HStack gap={3} align="start" w="100%">
        <Box
          ...
          border="2px solid"
          borderColor={
            taskd.status ? "blue.400" : isOverdue ? "red.400" : "gray.400"
          }
          bg={taskd.status ? "blue.400" : "transparent"}
          cursor="pointer"
          flexShrink={0}
          mt="3px"
          onClick={onToggle}
        ...
        <Box flex="1" minW={0}>
          <Flex justify="space-between" align="start" gap={2} mb={0.5}>
            <Text
              ...
              fontWeight="medium"
              fontSize="sm"
              textDecoration={taskd.status ? "line-through" : "none"}
              lineHeight="1.2"
